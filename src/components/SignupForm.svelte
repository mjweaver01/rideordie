...
<div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
  <div class="max-w-md w-full space-y-8 p-6 rounded-xl shadow-lg bg-white">
    <div>
      <h1 class="mt-6 text-center text-3xl font-extrabold text-black">Sign up</h1>
      <p class="mt-2 text-center text-sm text-black/80">
        Already have an account?
        <a href="/signin" class="font-medium text-black hover:text-black/70"> Sign in </a>
      </p>
    </div>
    <form class="mt-8 space-y-6" on:submit|preventDefault={createAccount}>
      <div class="rounded-md shadow-sm space-y-4">
        <div>
          ...
          <Input bind:value={name} type="text" id="name" placeholder="Name" autocomplete="name" />
        </div>
        <div>
